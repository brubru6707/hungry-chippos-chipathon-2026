...
C {dff_rst_n.sym} 50 -20 0 0 {name=x1}
C {vsource.sym} -200 100 0 0 {name=VRST value="PWL(0 0 30n 0 31n 3.3 374n 3.3 375n 0 425n 0 426n 3.3 750n 3.3)" savecurrent=false}
C {vsource.sym} -270 100 0 0 {name=VCLK value="PULSE(0 3.3 50n 1n 1n 40n 100n)" savecurrent=false}
C {vsource.sym} -350 100 0 0 {name=VDATA value="PWL(0 0 79n 0 80n 3.3 179n 3.3 180n 0 279n 0 280n 3.3 399n 3.3 400n 0 479n 0 480n 3.3 579n 3.3 580n 0 750n 0)" savecurrent=false}
...
"}
C {code_shown.sym} 330 -70 0 0 {name=COMMANDS only_toplevel=false value="
...
.control
  ...
  meas tran tcq_rise TRIG v(CLK) VAL=1.65 RISE=1 TD=100n TARG v(Q) VAL=1.65 RISE=1 TD=150n
  meas tran tcq_fall TRIG v(CLK) VAL=1.65 RISE=1 TD=200n TARG v(Q) VAL=1.65 FALL=1 TD=250n
  meas tran t_async_rst TRIG v(RST_N) VAL=1.65 FALL=1 TD=300n TARG v(Q) VAL=1.65 FALL=1 TD=374n

  let failures = 0

  if q020 > 0.33
    echo FAIL_q020_reset_did_not_force_Q_low
    let failures = failures + 1
  end

  if q070 > 0.33
    echo FAIL_q070_Q_should_be_low_after_first_edge
    let failures = failures + 1
  end
  ...
  if q370 < 2.97
    echo FAIL_q370_Q_did_not_capture_high
    let failures = failures + 1
  end

  if q385 > 0.33
    echo FAIL_q385_async_reset_did_not_force_Q_low
    let failures = failures + 1
  end

  if q415 > 0.33
    echo FAIL_q415_Q_did_not_remain_reset
    let failures = failures + 1
  end

  if q440 > 0.33
    echo FAIL_q440_Q_changed_when_reset_was_released
    let failures = failures + 1
  end
  ...
  print tcq_rise tcq_fall t_async_rst
  echo

  if failures < 0.5
    echo PASS_tb_dff_rst_n_all_functional_checks_passed
  else
    echo FAIL_tb_dff_rst_n
    print failures
  end

  write tb_dff_rst_n.raw
  plot v(CLK) v(D) v(RST_N) v(Q)
.endc
"}
