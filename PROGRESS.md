...
| DAC-10a | Top-level power + signal routing (bits 0-6 of 8) | Bruno | 🟡 In Progress | `dac/layout/dac_top_floorplan.gds`, `designs/scripts/gen_dac_top_floorplan.py` — bit-4's original single-bit routing template replaced with a general, per-bit-parametrized version and replicated to bits 0,1,2,3,5,6 (**7 of 8 bits now routed**; bit 7's oversized `m=2` driver needs its own dogleg, deferred). Variant-D full-block DRC **0 violations / 660 categories**. Per-bit continuity (B\<n\>→NAND-A, SAMPLE_N→NAND-B, NAND-Y→driver-gate, driver-VOUT→rail, all 4 supply drops) verified PASS for all 7 routed bits via `designs/scripts/check_all_bits.py`. A cross-bit distinctness check surfaced a `cap_array`-internal geometric artifact (reproduced with zero top routing present, in `cap_array.gds` alone) that a real LVS run — not yet performed — will need to resolve; see `dac/WORKLOG.md` 2026-07-19. |
