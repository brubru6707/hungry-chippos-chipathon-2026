...
| DAC-10a | Top-level power + signal routing (bits 0-6 of 8) | Bruno | 🟡 In Progress | 7 of 8 bits are routed; bit 7 still needs its dedicated dogleg. Variant-D full-block DRC is **0 violations / 660 categories** and routed endpoint continuity passes. The fixed via-cut checker now confirms pristine `cap_array` has **8 distinct rails**, matching caps-only LVS; however, it reports **28 existing unexpected cross-net merges** in the 0–6 top routing. Cross-bit distinctness is therefore blocking, and bit 7 must not be claimed complete. |
