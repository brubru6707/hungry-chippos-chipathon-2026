...
**DAC physical integration (2026-07-19):** top-level routing is complete. SAMPLE drives inv1 plus the TG NFET gate; inv1 output drives the shared SAMPLE_N trunk, all eight NAND-B pins, and the TG PFET gate; VIN is at the TG signal terminal; TG DAC_TOP lands on the M5 cap-top mesh. `designs/scripts/check_all_bits.py` reports all global and per-bit signal/supply continuity PASS and all **30** tracked nets pairwise distinct (**0/435 merges**); pristine `cap_array` remains 8 distinct rails. Full-block variant-D DRC is **0/660**. Real-reference LVS remains required for Gate 5.
