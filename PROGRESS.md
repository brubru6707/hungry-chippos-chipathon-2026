...
| DAC-10a | Top-level power + signal routing (bits 0-6 of 8) | Bruno | 🟡 In Progress | 7 of 8 bits are routed; bit 7 still needs its dedicated dogleg. The via-cut checker confirms pristine `cap_array` has **8 distinct rails** and now reports **all 24 tracked top-level nets pairwise distinct (0 merges)**, while every bit's four signal paths and VDD/0 drops are continuous. Variant-D full-block DRC: **0 violations / 660 categories**. Bit 7 and real-reference LVS remain outstanding, so this is not full routing closure. |
