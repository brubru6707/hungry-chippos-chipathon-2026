...
| DAC-9b | Capacitor-mismatch Monte Carlo INL/DNL (the real linearity limit) | Bruno | 🔴 **FAIL @ Cu=50fF** | `designs/scripts/dac_mismatch_mc.py`, `dac_mismatch_mc_spice.py`, `dac/docs/figures/mc_*.png` — PDK `cap_mim` MC term (2.5%/unit-cap, conservative global-variation proxy, gf180mcuD `mimcap_statistical`) gives yield **71.5%** at Cu=50fF (mean max\|DNL\|=0.42 LSB, close to the 0.5 LSB spec); worst code is the MSB major carry (0x7F→0x80), as expected. Transistor-level ngspice cross-check (N=50/transition) confirms the idealized model within 5-15%. **Recommendation: Cu ≥ 200-400 fF (4-8× current area) for ≥98.7% yield**, plus mandatory common-centroid layout (DAC-5) for systematic-gradient cancellation. See `dac/WORKLOG.md` Step 2 entries. |
| DAC-5 | Unit-cell layout with common-centroid placement (KLayout) | TBD | ⚪ Not Started | `dac/layout/cu_cell.gds` — **per DAC-9b, target Cu ≥ 200-400 fF (up from the schematic's 50 fF), common-centroid placement is mandatory for systematic-gradient cancellation, not optional** |
...
| Gate 3 | Top-level DNL/INL < 0.5 LSB @ TT corner | Integration | 🔴 DAC-only nominal sweep PASS (max\|DNL\|=0.007 LSB, max\|INL\|=0.008 LSB, see DAC-9), but cap-mismatch — the real linearity limit — **FAILS at Cu=50fF (71.5% yield, see DAC-9b)**; need Cu≥200-400fF + common-centroid layout. Full ADC-level (with comparator + sequencing) integration still ⚪ |
