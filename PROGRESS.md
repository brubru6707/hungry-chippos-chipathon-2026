...
| DAC-6 | Full array layout with dummy/fringe peripheral caps | TBD | 🟡 Partial | `dac/layout/cap_array.gds` — 255-unit common-centroid array (16×16 core, `PITCH_X=7.7µm`/`PITCH_Y=9.56µm`, set by MIMTM.1's 1.2µm bottom-plate spacing min) + 1-cell-wide dummy ring (69 dummy instances), all 324 instances referencing the single DAC-5 `dac_cap_unit` cell (no per-position redraw). 128 point-symmetric pairs: 127 interspersed (bit-reversal order) across B7..B1 give an **exact** zero centroid deviation for B1..B7 by construction; the physically most-central pair is B0+1 dummy, B0 deviation 6.14µm (smallest possible on a 16×16 grid, expected/acceptable for the LSB). Array DRC clean: **0 violations, 660 rule categories** (`run_dac_drc.sh`, variant=D). **Placement + centroid-verification checkpoint only — routing (DAC_TOP mesh, B0..B7 bit rails, dummy-ring ground ties) and array LVS not yet done**, see `dac/WORKLOG.md` 2026-07-18 "DAC-6 Step A" entry. |
