...
**DAC physical integration (2026-07-19):** all eight per-bit chains are complete. Bit 7’s m=2 driver uses a dedicated M3 escape/M4 dogleg below the shared SAMPLE_N trunk; all others use the established indexed lanes. `designs/scripts/check_all_bits.py` reports every signal/supply continuity check PASS and all 27 tracked nets pairwise distinct (0 merges); pristine `cap_array` remains 8 distinct rails. Full-block variant-D DRC is 0/660. Real-reference LVS remains required for Gate 5.
