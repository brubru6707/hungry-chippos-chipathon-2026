...
F {}
E {}
T {Gate-truth-table testbench (SAMPLE-gating verification). VREF=VDD=3.3V
rework (2026-07-18): unit_switch now takes a single control input bN_bar
= NAND2(B,SAMPLE_N) (net BxNAND), driving both the PMOS pull-up (to VDD)
and NMOS pull-down (to GND) gates directly - the old separate bN=AND
(B,SAMPLE_N) signal (BxG, generated by a now-removed extra inverter
...
Probes x1.B0NAND (bit0) and x1.B7NAND (bit7), i.e. bN_bar itself. Expect:
SAMPLE=1 => bN_bar HIGH regardless of B (bottom plate forced to GND,
PMOS off/NMOS on); SAMPLE=0 => bN_bar = NOT B (bottom plate follows the
bit: B=1 -> bN_bar=0 -> PMOS on -> VDD; B=0 -> bN_bar=1 -> NMOS on ->
GND).} 0 -300 0 0 0.25 0.25 {}
...
save all
tran 0.01n 40n
* bit0 truth table (bN_bar = x1.B0NAND)
meas tran b0nand_s1_b0 FIND v(x1.B0NAND) AT=5n
meas tran b0nand_s1_b1 FIND v(x1.B0NAND) AT=15n
meas tran b0nand_s0_b0 FIND v(x1.B0NAND) AT=25n
meas tran b0nand_s0_b1 FIND v(x1.B0NAND) AT=35n
* bit7 truth table (bN_bar = x1.B7NAND)
meas tran b7nand_s1_b0 FIND v(x1.B7NAND) AT=5n
meas tran b7nand_s1_b1 FIND v(x1.B7NAND) AT=15n
meas tran b7nand_s0_b0 FIND v(x1.B7NAND) AT=25n
meas tran b7nand_s0_b1 FIND v(x1.B7NAND) AT=35n
echo bit0_bN_bar_truth_table_V_s1b0_s1b1_s0b0_s0b1:
print b0nand_s1_b0 b0nand_s1_b1 b0nand_s0_b0 b0nand_s0_b1
echo bit7_bN_bar_truth_table_V_s1b0_s1b1_s0b0_s0b1:
print b7nand_s1_b0 b7nand_s1_b1 b7nand_s0_b0 b7nand_s0_b1
* bottom-plate (VOUT) node sanity: bit0 net1, bit7 net8
meas tran vout0_s0_b1 FIND v(x1.net1) AT=35n
meas tran vout7_s0_b1 FIND v(x1.net8) AT=35n
meas tran vout0_s0_b0 FIND v(x1.net1) AT=25n
meas tran vout7_s0_b0 FIND v(x1.net8) AT=25n
echo bottom_plate_V_bit0_bit7_at_S0B1_expect_VDD:
print vout0_s0_b1 vout7_s0_b1
echo bottom_plate_V_bit0_bit7_at_S0B0_expect_GND:
print vout0_s0_b0 vout7_s0_b0
.endc
"}
