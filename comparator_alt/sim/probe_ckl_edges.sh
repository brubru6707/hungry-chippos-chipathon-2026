#!/bin/bash
# probe_ckl_edges.sh — boundary probe: locate the ss/3.0 floor and FF ceiling
...
set -e
NETLIST="$1"
OUTDIR="/foss/designs/comparator_alt/sim/alt11_probe_edges"
...
# ss/3.0 floor: failing at 2.8, passing at 3.2 -> where exactly?
for ckl in 2.9 3.0 3.1; do
...
# FF ceiling: passing at 3.2, dead at 3.6 -> where exactly? (0.5LSB and 0.1mV od)
for ckl in 3.3 3.4 3.5; do
  v1c=$(awk "BEGIN{printf \"%.6f\", 3.6/2 - 3.6/512}")
  gen "ffB_bal_ckl${ckl}" ff -40 3.6 "$v1c" "$ckl" 1
done
for ckl in 3.2 3.3 3.4; do
  v1e=$(awk "BEGIN{printf \"%.6f\", 3.6/2 - 0.0001}")
  gen "ffB_tiny_ckl${ckl}" ff -40 3.6 "$v1e" "$ckl" 1
done
...
echo "===================================================="
echo "Edges: last ssA WRONGPOL = floor; first ffB NODECIDE = ceiling. Window = between them."
