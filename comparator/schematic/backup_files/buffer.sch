...
C {vsource.sym} 770 -730 0 0 {name=VVIN1 value="PWL(0 1.1 60n 1.3)" savecurrent=false}
...
while run <= mc_runs
	...
	* FIX 1: Create a difference vector to avoid the 'vexprint1' parsing bug
	let vdiff = v(out1) - v(out2)

	* Force evaluation using the new difference vector
	meas tran trip_time when vdiff=0 cross=1

	* FIX 2: Evaluate the run variable to an explicit integer
	let idx = $&run - 1
	...
	if $?trip_time
		...
		* FIX 3: Force integer evaluation of idx using $&
		let offset_results[$&idx] = {$current_tran}.current_offset

	else
		print 'Warning: Comparator did not trip on run $&run'
