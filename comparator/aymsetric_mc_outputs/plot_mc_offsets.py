...
def parse_ngspice_raw(filepath):
    ...
    # unpack float64s (interleaved: [var0_pt0, var1_pt0, var0_pt1, ...])
...
offsets_v = data[:, 1]          # offset_results column
offsets_mv = offsets_v * 1e3    # convert to mV
...
print(f"  3σ    : [{n3sigma:+.3f}, {p3sigma:+.3f}] mV")
print(f"  Min   : {offsets_mv.min():+.3f} mV")
print(f"  Max   : {offsets_mv.max():+.3f} mV")

fig, axes = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle("Comparator MC Offset Analysis", fontsize=14, fontweight="bold")

# histogram
ax = axes[0]
n, bins, _ = ax.hist(offsets_mv, bins=30, color="steelblue", edgecolor="white", alpha=0.85)
